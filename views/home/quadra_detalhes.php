  ...
  <section>
    ...
    <div class="container">
      ...
      <h3><?php echo $quadra['coberta'] ? 'Quadra coberta' : 'Quadra descoberta'; ?>,
        <?php echo htmlspecialchars($quadra['descricao_proprietario']); ?></h3>
      <div>
        <div id="grid-reserva">
        ...
        < <script>
          ...
          fetch(
          `../../controllers/AuthController.php?action=getHorariosDisponiveis&quadra_id=<?= $id_quadra ?>&data=${this.value}`
          )
          ...
          while (inicio < fim) { let horaFormatada=inicio.toTimeString().slice(0, 5); let
            optionInicio=document.createElement('option'); optionInicio.value=horaFormatada;
            optionInicio.textContent=horaFormatada; horarioInicio.appendChild(optionInicio);
            inicio.setMinutes(inicio.getMinutes() + 30); } }); } horarioInicio.addEventListener('change', function() {
            horarioFim.innerHTML='<option value="">Fim</option>' ; if (this.value) { let inicio=new
            Date(`2000-01-01T${this.value}`); let options=horarioInicio.querySelectorAll('option'); let
            encontrouInicio=false; options.forEach(option=> {
